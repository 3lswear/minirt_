	...
	if (scene->lights)
	{
		if (closest->type == T_SPHERE)
		{
			...
			color = c_mul(closest->color, c_add(ambient, calc_lights_2s
						(closest->obj.plane.norm, scene, ray_min, cam)));
		else if (closest->type == T_TRIANG)
			color = c_mul(closest->color, c_add(ambient, calc_lights_2s
						(closest->obj.triang.norm, scene, ray_min, cam)));
		else if (closest->type == T_SQUARE)
			color = c_mul(closest->color, c_add(ambient, calc_lights_2s
						(closest->obj.square.norm, scene, ray_min, cam)));
		else if (closest->type == T_CYLIND)
		{
			color = c_mul(closest->color, c_add(ambient, calc_lights_2s
						(closest->obj.cylind.cur_norm, scene, ray_min, cam)));
